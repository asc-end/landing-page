...
export default function Friends() {
    ...
    return (
        <section className="flex flex-col w-full gap-12 py-44 items-center justify-center relative overflow-visible px-60" ref={containerRef}>
            ...
            {/* <h1 className=" pointer-events-none">Play with your friends  </h1> */}
            {sortedFriends.map(friend =>
                <Friend friend={friend.name} animationPercentage={animationPercentage} key={friend.name} {...friend} mousePos={mousePos} parallax={windowScroll.y - top + (rect?.height ? (rect.height / 2) : 0)} />
            )}
        </section>
    )
}